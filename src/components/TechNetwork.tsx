...
import { SiTypescript, SiJavascript, SiNodedotjs, SiReact, SiNextdotjs, SiExpress, SiCplusplus, SiPython, SiDjango, SiFlask, SiTensorflow } from "react-icons/si";
...
export default function TechNetwork() {
  ...
  const sidebarPointerEvents = useTransform(scrollY, [400, 600, 3400, 3600], ['none', 'auto', 'auto', 'none']) as any;

  // Transformaciones para el Menú Lateral
  const menuOpacity1 = useTransform(scrollY, [400, 600, 1000, 1200], [0.3, 1, 1, 0.3]);
  const menuOpacity2 = useTransform(scrollY, [1200, 1400, 1800, 2000], [0.3, 1, 1, 0.3]);
  const menuOpacity3 = useTransform(scrollY, [2000, 2200, 2600, 2800], [0.3, 1, 1, 0.3]);
  const menuOpacity4 = useTransform(scrollY, [2800, 3000, 3400, 3600], [0.3, 1, 1, 0.3]);
  
  const menuScale1 = useTransform(scrollY, [400, 600, 1000, 1200], [1, 1.2, 1.2, 1]);
  const menuScale2 = useTransform(scrollY, [1200, 1400, 1800, 2000], [1, 1.2, 1.2, 1]);
  const menuScale3 = useTransform(scrollY, [2000, 2200, 2600, 2800], [1, 1.2, 1.2, 1]);
  const menuScale4 = useTransform(scrollY, [2800, 3000, 3400, 3600], [1, 1.2, 1.2, 1]);
  ...
  const nodes1 = [
    ...
    { id: 4, icon: SiJavascript, x: 30, y: 70, color: '#F7DF1E', name: "JavaScript" },
    { id: 5, icon: SiNextdotjs, x: 70, y: 75, color: '#ffffff', name: "Next.js" },
  ];
  const conn1 = [[0, 2], [1, 2], [2, 3], [2, 4], [0, 3], [1, 4]];
  ...
  const nodes3 = [
    ...
    { id: 3, icon: SiCplusplus, x: 80, y: 70, color: '#00599C', name: "Unreal" },
  ];
  const conn3 = [[0, 1], [0, 2]];
  ...
  const nodes4 = [
    ...
    { id: 4, icon: SiTensorflow, x: 50, y: 80, color: '#FF6F00', name: "TensorFlow" },
  ];
  const conn4 = [[0, 1], [0, 2], [0, 3], [1, 2]];
  ...
  return (
    <div style={{
      ...
      <motion.div style={{
        ...
        <motion.div style={{ opacity: menuOpacity1, scale: menuScale1, transformOrigin: 'left', cursor: 'pointer' }} onClick={() => window.scrollTo({ top: 800, behavior: 'smooth' })}>
          <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', letterSpacing: '2px' }}>01_ WEB</span>
        </motion.div>
        <motion.div style={{ opacity: menuOpacity2, scale: menuScale2, transformOrigin: 'left', cursor: 'pointer' }} onClick={() => window.scrollTo({ top: 1600, behavior: 'smooth' })}>
          <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', letterSpacing: '2px' }}>02_ JAVA</span>
        </motion.div>
        <motion.div style={{ opacity: menuOpacity3, scale: menuScale3, transformOrigin: 'left', cursor: 'pointer' }} onClick={() => window.scrollTo({ top: 2400, behavior: 'smooth' })}>
          <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', letterSpacing: '2px' }}>03_ C++</span>
        </motion.div>
        <motion.div style={{ opacity: menuOpacity4, scale: menuScale4, transformOrigin: 'left', cursor: 'pointer' }} onClick={() => window.scrollTo({ top: 3200, behavior: 'smooth' })}>
          <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', letterSpacing: '2px' }}>04_ PYTHON</span>
        </motion.div>
      </motion.div>
      ...
      {ecosystems.map((eco, index) => (
        <div key={index} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}>
          <motion.svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: eco.l }}>
          ...
          <motion.div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: eco.o, y: eco.y, scale: eco.s }}>
            {eco.nodes.map(node => {
